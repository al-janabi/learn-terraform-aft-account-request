...
module "network" { #Create a new module resource for every new account you want to provision ,You can  define module name as per your requirement
  ...
  control_tower_parameters = {
    AccountEmail              = "ajanabi+nwk@amazon.com" #Account email must be unique and you should be able to verify account through email, Usage :- user+accountname@domain
    AccountName               = "nwk" # Account Name
    ManagedOrganizationalUnit = "Managment (ou-mmqs-a4hvskr6)" #if you are using nested OUs, include the OU ID in parentheses, such as Sandbox (ou-44...)
  ...
  custom_fields = { #Custom fields are use to store defined attributes in parameter store in account provisioned as key value pair ,This parameters while be utilized if required while executing code from account customizations
    ...
    prd_ipam_cidr = "10.101.0.0/16"  #Set IPAM CIDR as per your requirement, This attribute will be stored in parameter Store of newly network account created
    dev_ipam_cidr = "10.100.0.0/16"  #Set IPAM CIDR as per your requirement, This attribute will be stored in parameter Store of newly network account created
  }
